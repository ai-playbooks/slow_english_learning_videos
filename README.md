...
- **Right source frame, every clip (v7.1).** Create Video fires only when the candidate carrying that beat's keyframe is the one selected item in the strip, and every finished clip's first frame is numerically compared with its keyframe (a mismatch is resubmitted once, a second mismatch is reported and the clip is left out). Added after a Codex run animated 11 of 13 clips from the same keyframe, so the film reset to the opening pose every few seconds.
